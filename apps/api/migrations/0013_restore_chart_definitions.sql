-- Recreate the persisted chart resource removed by 0012.
-- Keep 0011 and 0012 immutable so fresh and upgraded databases converge here.
CREATE TABLE IF NOT EXISTS riviamigo.charts (
...
CREATE UNIQUE INDEX IF NOT EXISTS charts_system_slug_idx
    ON riviamigo.charts (slug)
    WHERE owner_id IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS charts_owner_slug_idx
    ON riviamigo.charts (owner_id, slug)
    WHERE owner_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS charts_owner_enabled_idx
