//! Read-only charge-session diagnostics for canonicalization work.
//!
//! Usage:
//!   cargo run --bin diagnose_charge_sessions -- [--vehicle <uuid>]

use anyhow::{anyhow, Context, Result};
use riviamigo_api::services::charge_sessions::diagnose_charge_sessions;
...
fn parse_args() -> Result<Args> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!("Usage: cargo run --bin diagnose_charge_sessions -- [--vehicle <uuid>]");
                std::process::exit(0);
            }
