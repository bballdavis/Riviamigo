...
pub struct TimeSeriesPoint { pub ts: DateTime<Utc>, pub value: f64 }

fn resolution(from: DateTime<Utc>, to: DateTime<Utc>) -> &'static str {
    let hours = (to - from).num_hours();
    if hours <= 48 { "1min" } else if hours <= 2160 { "1hr" } else { "1day" }
}
...
async fn get_soc(
    ...
    let from = p.from.unwrap_or_else(|| Utc::now() - chrono::Duration::days(7));
...
async fn get_range(
    ...
    let vid  = p.vehicle_id.ok_or(AppError::Validation("vehicle_id required".into()))?;
    require_vehicle_owned(&state.pool, auth.user_id, vid).await?;
    let from = p.from.unwrap_or_else(|| Utc::now() - chrono::Duration::days(30));
    let to   = p.to.unwrap_or_else(Utc::now);
...
async fn get_phantom_drain(
    ...
    let points = sqlx::query_as!(PhantomDrainPoint,
        "SELECT day, total_soc_lost, avg_drain_rate, total_hours_parked AS hours_parked \
