...
async fn list_trips(
    ...
    let rows = sqlx::query_as!(TripRow,
...
async fn get_track(
    ...
    let bucket = if duration_min < 15 { "1 second" } else if duration_min < 60 { "5 seconds" } else { "15 seconds" };
...
async fn get_speed_profile(
    ...
    Ok(Json(serde_json::json!(points.iter().map(|r| serde_json::json!({"ts":r.ts,"value":r.value})).collect::<Vec<_>>())))
}
