...
async fn deleting_theme_owner_cleans_history_but_direct_revision_mutation_is_rejected() {
    ...
    assert!(direct_delete
        ...
        .to_string()
        .contains("append-only"));
    let direct_theme_delete = sqlx::query("DELETE FROM riviamigo.user_themes WHERE id = $1")
        .bind(theme_id)
        .execute(&app.pool)
        .await;
    assert!(direct_theme_delete
        .expect_err("direct theme delete must be rejected")
        .to_string()
        .contains("must be retired"));
