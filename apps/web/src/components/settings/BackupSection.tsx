...
const RESTORE_BLOCKING_ERROR_COPY: Record<string, { title: string; message: string }> = {
...
function restoreBlockingErrorCopy(error: RestorePlan['blocking_errors'][number]) {
  const code = String(error.code);
  return {
    code,
    ...(RESTORE_BLOCKING_ERROR_COPY[code] ?? {
      title: 'Restore compatibility check failed',
...
function storageLabel(
  storageType: BackupOverview['artifacts'][number]['storage_type'],
  manifest?: BackupOverview['artifacts'][number]['manifest']
) {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 truncate font-mono text-xs font-semibold leading-none text-fg">
          {value}
        </p>
      </div>
    </div>
...
function ToggleRow({
  ...
  return (
    ...
    >
      <div>
        <p className={`text-sm font-medium ${disabled ? 'text-fg-tertiary' : 'text-fg'}`}>
          {title}
        </p>
...
export function BackupSection() {
  ...
  const [pendingRestoreArtifact, setPendingRestoreArtifact] = React.useState<
    BackupOverview['artifacts'][number] | null
  >(null);
  ...
  const [activeRestore, setActiveRestore] = React.useState<{
    job: RestoreJob;
    token: string;
  } | null>(null);
  ...
  const runBackupNow = useMutation({
    ...
    onError: (error) => {
      emitToast(
        'Backup run',
        error instanceof Error ? error.message : 'Backup run could not be started.'
      );
    },
  });
  ...
  const testS3 = useMutation({
    ...
    onSuccess: ({ message }) =>
      window.dispatchEvent(
        new CustomEvent('riviamigo:toast', {
          detail: { title: 'S3 connection', message, variant: 'success' },
        })
      ),
    onError: (error) =>
      emitToast(
        'S3 connection',
        error instanceof Error ? error.message : 'The S3 connection test failed.'
      ),
  });
  ...
  const downloadArtifact = useMutation({
    ...
    onError: (error) => {
      emitToast(
        'Download backup',
        error instanceof Error ? error.message : 'Backup download could not be started.'
      );
    },
  });
  ...
  const uploadArtifact = useMutation({
    mutationFn: (file: File) =>
      api.uploadBackupArtifact(file, (loaded, total) => {
    ...
    onError: (error) => {
      ...
      emitToast(
        ...
        error instanceof Error ? error.message : 'The recovery package could not be imported.'
      );
    },
  });
  ...
  const deleteUploadedArtifact = useMutation({
    ...
    onError: (error) =>
      emitToast(
        ...
        error instanceof Error ? error.message : 'The imported package could not be deleted.'
      ),
  });

  const preflightRestore = useMutation({
    mutationFn: (artifact: BackupOverview['artifacts'][number]) =>
      api.preflightBackupRestore(artifact.id),
    ...
    onError: (error) =>
      emitToast(
        ...
        error instanceof Error ? error.message : 'The recovery package could not be inspected.'
      ),
  });
  ...
  const requestRestore = useMutation({
    ...
    onError: (error) => {
      emitToast(
        'Restore request',
        error instanceof Error ? error.message : 'Restore request could not be created.'
      );
    },
  });
  ...
  React.useEffect(() => {
    if (
      !activeRestore ||
      activeRestore.job.phase === 'completed' ||
  ...
  const s3Valid =
    ...
    (draft.bucket.trim().length > 0 &&
      ...
      (currentSettings.has_secret_key || secretKey.trim().length > 0));
  const canSave =
    draft.run_at.trim().length === 5 &&
  ...
  const hasLocalArtifacts = overview.data.artifacts.some(
    (artifact) => (artifact.storage_type as string) !== 's3'
  );
  const hasS3Artifacts = overview.data.artifacts.some(
    (artifact) => (artifact.storage_type as string) === 's3'
  );
  ...
  const availableCatalogSources = [
    ...(hasLocalCatalog && (hasS3Artifacts || s3Configured)
  ...
  const defaultCatalogSource =
    hasLocalCatalog && hasS3Artifacts ? 'all' : hasS3Artifacts && !hasLocalCatalog ? 's3' : 'local';
  const selectedCatalogSource =
    catalogSource && availableCatalogSources.some((source) => source.value === catalogSource)
      ? catalogSource
      : defaultCatalogSource;
  const visibleArtifacts = overview.data.artifacts.filter(
    (artifact) =>
      selectedCatalogSource === 'all' ||
      (selectedCatalogSource === 's3'
        ? (artifact.storage_type as string) === 's3'
        : (artifact.storage_type as string) !== 's3')
  );
      ...
      <Card>
        ...
        <CardContent className="grid gap-4">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            ...
            />
            <InfoChip icon={Calendar} label="Schedule" value={describeSchedule(overview.data)} />
            <InfoChip
              icon={CheckCircle2}
          ...
          <div className="grid gap-2 md:grid-cols-2">
            <ToggleRow
              title="Local backup"
              description="Retain each recovery package on this host"
              checked={draft.local_enabled}
              onChange={(value) => updateDraft('local_enabled', value)}
            />
            <ToggleRow
              title="S3 backup"
              description="Upload each package to off-site S3-compatible storage"
              checked={draft.s3_enabled}
              onChange={(value) => updateDraft('s3_enabled', value)}
            />
          </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-fg-tertiary">
                    Custom endpoints use path-style addressing for Garage, MinIO, and similar
                    providers.
                  </p>
                </div>
                <Button
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <label className="grid gap-1 md:col-span-2">
                  <span className="text-xs font-medium uppercase tracking-wide text-fg-tertiary">
                    Endpoint <span className="font-normal normal-case">(leave blank for AWS)</span>
                  </span>
                  <input
                ...
                {currentSettings.has_secret_key && (
                  <label className="flex items-center gap-2 text-xs text-fg-tertiary md:col-span-2">
                    <input
                      type="checkbox"
                      checked={clearSecretKey}
                      onChange={(event) => setClearSecretKey(event.target.checked)}
                    />
                    Clear the saved secret when settings are saved
                  </label>
                )}
              </div>
              {overview.data.s3_catalog_error && (
                <p className="text-xs text-status-warning">
                  S3 catalog unavailable: {overview.data.s3_catalog_error}
                </p>
              )}
            </div>
          )}
            ...
            {draft.frequency === 'monthly' && (
              <label className="grid gap-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateDraft('day_of_month', Number.parseInt(e.target.value, 10) || 1)
                  }
                  className="h-9 rounded-lg border border-border bg-bg-elevated px-3 text-sm text-fg outline-none focus:border-accent"
                />
              </label>
            ...
            <>
                  ...
                  {overview.data.recent_runs.map((run) => (
                    ...
                    >
                      <div className="flex min-w-0 flex-wrap items-center gap-2">
                        ...
                        {run.error_message && (
                          <span
                            className="min-w-0 truncate text-xs text-status-danger"
              ...
              <div className="flex shrink-0 items-center justify-between border-t border-border pt-3">
                <p className="text-xs text-fg-tertiary">
                  Page {overview.data.recent_runs_page} of {recentRunsPageCount} · {recentRunsTotal}{' '}
                  run{recentRunsTotal === 1 ? '' : 's'}
                </p>
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setRecentRunsPage((page) => Math.min(recentRunsPageCount, page + 1))
                    }
                  >
                    Next
                  </Button>
                  ...
                  >
                    <div className="flex items-center gap-3 px-3 py-2.5">
                      <button
                        type="button"
                        aria-label={
                          isExpanded ? 'Collapse backup details' : 'Expand backup details'
                        }
                      ...
                      <div className="flex shrink-0 items-center gap-1.5">
                        <Button
                          ...
                          loading={
                            ...
                            downloadArtifact.variables?.id === artifact.id
                          }
                          onClick={(event) => {
                            event.stopPropagation();
                        ...
                        {artifact.storage_type === 'uploaded' && (
                          <Button
                            ...
                            loading={
                              deleteUploadedArtifact.isPending &&
                              deleteUploadedArtifact.variables === artifact.id
                            }
                            onClick={(event) => {
                              event.stopPropagation();
                              if (
                                window.confirm(
                                  `Delete imported recovery package "${artifact.file_name}"?`
                                )
                              ) {
                      ...
                      <div className="border-t border-border bg-bg-elevated/25 px-3 py-3 text-xs text-fg-tertiary">
                        <div className="grid gap-3 md:grid-cols-2">
                          ...
                          <div>
                            ...
                            <p className="mt-1 font-mono text-xs text-fg capitalize">
                              {artifact.storage_type}
                            </p>
                          </div>
                          ...
                          <div>
                            ...
                            <p className="mt-1 font-mono text-xs text-fg">
                              {Math.max(artifact.size_bytes, 0).toLocaleString()} bytes
                            </p>
                          </div>
                          <div>
                            <p className="text-[10px] font-medium uppercase tracking-wider text-fg-tertiary">
                          ...
                          <div className="mt-4 border-t border-border pt-3">
                            ...
                            <div className="mt-2 grid gap-3 md:grid-cols-2">
                              <div>
                                ...
                                <p className="mt-1 font-mono text-xs text-fg">
                                  {artifact.manifest.package.format ?? 'Unknown'} v
                                  {artifact.manifest.package.format_version ?? '?'}
                                </p>
                              </div>
                              <div>
                                <p className="text-[10px] font-medium uppercase tracking-wider text-fg-tertiary">
                              ...
                              {(['included', 'redacted', 'excluded'] as const).map((scopeKey) => {
                                ...
                                return (
                                  <div key={scopeKey} className="md:col-span-2">
                                    <p className="text-[10px] font-medium uppercase tracking-wider text-fg-tertiary">
                                      {capitalizeFirstLetter(scopeKey)}
                                    </p>
                                    <p className="mt-1 text-xs text-fg">
                                      {values.join(' · ') || 'None listed'}
                                    </p>
                                  </div>
                                );
                              })}
                              <div className="md:col-span-2">
                                ...
                                <p className="mt-1 text-xs text-fg">
                                  {String(
                                    artifact.manifest.package.restore?.requires ??
                                      'Use the restore script'
                                  )}
                                  ; provider credentials require re-authentication.
                                </p>
                              </div>
                            </div>
                          </div>
          ...
          <div className="w-full max-w-lg rounded-2xl border border-border bg-bg-surface p-5 shadow-2xl">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-sm text-fg-tertiary">
                  This replaces the current users, dashboards, telemetry, settings, and artwork with{' '}
                  <span className="font-mono text-fg">{pendingRestoreArtifact.file_name}</span>.
                  Riviamigo creates a required safety package first, stops its API and ingestion
                  workers, restores the package, and starts cleanly on the restored data.
                </p>
              </div>
              <button
                type="button"
            ...
            {pendingRestorePlan && (
              <div className="mt-4 rounded-lg border border-border bg-bg-elevated/40 p-3 text-xs">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <Badge variant={pendingRestorePlan.compatible ? 'success' : 'danger'}>
                    {pendingRestorePlan.compatible ? 'Compatible' : 'Blocked'}
                  </Badge>
                </div>
                <p className="mt-2 text-fg-secondary">
                  Schema migration {pendingRestorePlan.source.migration_version} to{' '}
                  {pendingRestorePlan.target.migration_version}
                  {pendingRestorePlan.pending_migrations.length > 0
                    ? ` - ${pendingRestorePlan.pending_migrations.length} pending migration${pendingRestorePlan.pending_migrations.length === 1 ? '' : 's'}`
                    : ' - no pending migrations'}
                </p>
                {pendingRestorePlan.transforms.length > 0 && (
                  <p className="mt-1 text-fg-secondary">
                    Transforms:{' '}
                    {pendingRestorePlan.transforms.map((transform) => transform.id).join(', ')}
                  </p>
                )}
                {pendingRestorePlan.warnings.map((warning) => (
                  <p key={warning} className="mt-1 text-status-warning">
                    {warning}
                  </p>
                ))}
                {pendingRestorePlan.blocking_errors.map((error) => {
                  const copy = restoreBlockingErrorCopy(error);
                  return (
                    <div
                      key={`${copy.code}-${error.message}`}
                      className="mt-3 rounded-md border border-status-danger/30 bg-status-danger/10 p-2 text-status-danger"
                      data-testid={`restore-blocking-error-${copy.code}`}
                    >
                      <p className="font-medium">{copy.title}</p>
                      <p className="mt-1">{copy.message}</p>
                      {error.message !== copy.message && (
                        <p className="mt-1 text-status-danger/80">Details: {error.message}</p>
                      )}
            ...
            {!overview.data.runtime_readiness.restore_automation_available && (
              <p className="mt-3 text-xs text-status-warning">
                Automated restore is unavailable in this runtime. Use the documented restore script
                on the host.
              </p>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-bg-page/90 px-4 backdrop-blur-sm">
          ...
          >
            ...
            <div
              ...
              aria-valuetext={
                activeRestore.job.phase === 'failed' ? 'Restore failed' : 'Restore is running'
              }
              className="mt-4 h-2.5 overflow-hidden rounded-full bg-bg-elevated"
            >
              <div
                className={`h-full rounded-full ${activeRestore.job.phase === 'failed' ? 'bg-status-danger' : activeRestore.job.phase === 'completed' ? 'bg-status-success' : 'rm-restore-activity'}`}
              />
            </div>
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-fg-tertiary">
              ...
              {activeRestore.job.phase !== 'failed' && (
                <span>
                  Estimate based on package size; safety backup and database load can extend it.
                </span>
              )}
            </div>
            {restoreStatusUnavailable && activeRestore.job.phase !== 'failed' && (
              <div className="mt-4 rounded-lg border border-status-warning/30 bg-status-warning/10 p-3 text-sm text-status-warning">
                The application is restarting for the restore. The restore may continue while this
                page cannot reach the API; we&apos;ll keep checking and reload when the server
                returns. You may be asked to sign in again.
              </div>
            )}
            {activeRestore.job.validation_report && (
              <div className="mt-4 rounded-lg border border-status-positive/30 bg-status-positive/10 p-3 text-xs text-fg-secondary">
                ...
                <p className="mt-1">
                  Schema contract verified and foreign keys validated
                  {activeRestore.job.validation_report.migrations_applied.length > 0
                    ? ` - migrations applied: ${activeRestore.job.validation_report.migrations_applied.join(', ')}`
                    : ' - no migrations applied'}
                  {activeRestore.job.validation_report.applied_transforms.length > 0
                    ? ` - transforms: ${activeRestore.job.validation_report.applied_transforms.map((transform) => transform.id).join(', ')}`
                    : ''}
                </p>
              </div>
            )}
            {activeRestore.job.rollback_state &&
              activeRestore.job.rollback_state !== 'not_required' &&
              activeRestore.job.rollback_state !== 'available' && (
                <div
                  className={`mt-4 rounded-lg border p-3 text-sm ${activeRestore.job.rollback_state === 'succeeded' ? 'border-status-positive/30 bg-status-positive/10 text-status-positive' : 'border-status-warning/30 bg-status-warning/10 text-status-warning'}`}
                >
          ...
          <div className="-mx-5 rounded-lg border border-border bg-bg-elevated/20 p-5">
            <div>
              ...
              <p className="mt-1 text-xs leading-relaxed text-fg-tertiary">
                Select a backup already in the local catalog or import a package from another
                Riviamigo installation. A safety backup is created before any restore begins.
              </p>
            </div>
            ...
            {uploadArtifact.isPending && (
              <div
                ...
                className="mt-3 rounded-lg border border-border bg-bg-elevated/30 p-3"
              >
                <div className="flex items-center justify-between gap-3 text-xs text-fg-secondary">
                  <span>
                    {uploadProgress === 100
                      ? 'Validating recovery package…'
                      : 'Uploading recovery package…'}
                  </span>
            ...
            {restoreArtifact ? (
              <div className="mt-3 flex flex-col gap-3 rounded-lg border border-accent/30 bg-accent/5 p-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Button
                  ...
                  disabled={
                    !overview.data.runtime_readiness.restore_automation_available ||
                    !artifactIsAvailable(restoreArtifact)
                  }
              ...
              <p className="mt-3 text-xs text-fg-tertiary">
                No recovery packages are available yet.
              </p>
            )}

            {!overview.data.runtime_readiness.restore_automation_available && (
              <p className="mt-3 text-xs text-status-warning">
                Automated restore is unavailable
                {restoreAutomationReason ? `: ${restoreAutomationReason}` : ' in this runtime.'}
              </p>
            )}
          </div>
          ...
          <div className="border-t border-border pt-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="mt-0.5 text-xs text-fg-tertiary">
                  Previous restore attempts and their current status.
                </p>
              </div>
            </div>
            {overview.data.restore_requests.length === 0 ? (
              <p className="mt-3 text-sm text-fg-tertiary">
                No restore requests have been recorded yet.
              </p>
            ) : (
                  ...
                  >
                    <div>
                      ...
                      {request.error_message && (
                        <p className="mt-1 text-xs text-status-danger">{request.error_message}</p>
                      )}
                    </div>
