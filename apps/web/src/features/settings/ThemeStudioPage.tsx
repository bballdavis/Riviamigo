  ...
  return <div className="min-h-screen bg-bg-page text-fg">
    ...
    <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 lg:grid-cols-[13rem_minmax(0,1fr)] lg:px-6">
      ...
      <main className="min-w-0">
        ...
        {section === 'brand' ? <StudioSection title="Brand" description="Brand assets inherit from the built-in base until approved vector masters expose trusted paint slots."><BrandPreview resolved={resolved} /></StudioSection> : null}
...
function BrandPreview({ resolved }: { resolved: ReturnType<typeof resolveTheme> }) { return <div className="grid gap-4 rounded-xl border border-border bg-bg-elevated/35 p-4 sm:grid-cols-2"><div className="grid min-h-36 place-items-center rounded-lg border border-border bg-bg-page p-5"><img src={resolved.brandAssets.wordmark.dark} alt="Riviamigo wordmark preview" className="max-h-16 max-w-full" /></div><div className="grid content-center gap-2"><p className="text-sm font-medium text-fg">Raster-backed fallback</p><p className="text-xs leading-relaxed text-fg-secondary">The current logo and wordmark sources embed raster artwork. Their geometry and built-in treatment are inherited unchanged; custom paint editing stays locked until self-contained vector masters pass visual approval.</p></div></div>; }
