...
The chart host resolves fixed or assigned catalogs and passes the active record into one shared chart frame. Overview therefore uses the same picker, settings panel, measured plot, mobile viewer, focus restoration, favorite behavior, and chart renderer as fixed dashboard catalogs. Assignment metadata such as `placements` only controls eligibility. A known bundled slug always uses its established dashboard renderer on every page; custom slugs use `ChartDefinitionRenderer`. Editor previews pass their current unsaved record through this same production runtime.
