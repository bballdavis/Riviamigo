...
The chart host resolves fixed or assigned catalogs and passes the active record into one shared chart frame. Overview therefore uses the same picker, settings panel, measured plot, mobile viewer, focus restoration, and favorite behavior as fixed dashboard catalogs. Renderer compatibility is declared per bundled chart and based only on render-affecting definition fields: assignment metadata such as `placements` cannot force a bundled chart onto a different renderer. Current specialized components declare no persisted definition-level overrides—their range and smoothing controls are widget settings—so any saved visual edit falls back to `ChartDefinitionRenderer` and is honored completely. Custom definitions always use the generic path. Editor previews pass their current unsaved record through this same production runtime.
