...
- The chart editor is curve-first. Its ordinary sections are Basics, Curves, Display, and Advanced; source bindings and raw domain fields are implementation details, not primary navigation. The first curve establishes the shared domain, incompatible curves stay searchable with a plain explanation, and uncommon data controls use progressive disclosure inside the affected curve.
