...
- Long-running destructive operations use a confirmation dialog with explicit impact copy and a typed phrase, followed by one viewport-bounded progress surface. Report byte progress only when measurable and use named phases for server work; keep failure details and recovery actions visible instead of implying false precision.
