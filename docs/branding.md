...
- Eligible line and area views also expose **Curve smoothness** with `Straight`, `Gentle`, and `Smooth`. This is renderer-only path shaping: it preserves recorded timestamps, tooltip values, point counts, and null gaps. Curved paths use shape-preserving monotone interpolation so the display cannot invent peaks between telemetry samples; bars, scatter, categories, mileage, and trip-detail views do not expose it.
