...
- API error shape: `{ error: { code, message } }` (per [project memory](../../.claude/projects/-Users-<forgejo-owner>-Repos-Riviamigo/memory/project_riviamigo.md)).
...
- **Integration (Rust)**: CRUD endpoints, role gating (non-admin cannot edit defaults), clone copies config, slug uniqueness per-owner. Use existing testcontainers Postgres harness — see [project memory](../../.claude/projects/-Users-<forgejo-owner>-Repos-Riviamigo/memory/project_riviamigo.md).
