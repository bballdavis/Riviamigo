...
5. Supporting poll flows reconcile completed charging sessions and live charge-curve data into canonical `charge_sessions`, preserving telemetry-backed windows as the public session timeline while storing Rivian aliases and API-only history as enrichment evidence. While a vehicle is actively charging, the same poll loop fetches the normalized live-session payload and writes `vehicle:{vehicle_id}:live_session` to Redis with a 120-second TTL. The key is deleted when charging ends or the upstream reports no active session; transient polling failures leave the existing value to expire naturally.
