...
Trip geometry remains exact. CARTO tiles are requested through an authenticated Riviamigo proxy, so the tile provider sees the requested map area and the server connection, not each viewer's browser identity. Administrators can add, replace, or explicitly clear a write-only, encrypted CARTO API key in the Map basemap connection; Riviamigo sends it only from the server to CARTO or a Carto-hosted custom template, never to an arbitrary custom tile provider or back to a browser. Use a minimally privileged [CARTO API Access Token](https://docs.carto.com/carto-for-developers/key-concepts/authentication-methods/api-access-tokens) for this purpose. Without a key, remote CARTO maps still render with CARTO's watermark and show an informational notice explaining where an administrator can add one. Custom XYZ raster templates require `{z}`, `{x}`, and `{y}`, attribution, and may include an encrypted bearer token. A missing dark template falls back to the light template.
