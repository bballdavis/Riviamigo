...
Riviamigo does not send product analytics. These connections exist only to provide a feature you request. Each card shows the destination, data sent, last sanitized result, daily request count, and what stops if the connection is disabled. **Disable optional** stops weather, geocoding, basemaps, remote icon catalog access, and new Rivian artwork downloads without deleting stored results.
...
Hosted Open-Meteo is enabled on upgrade to preserve existing behavior. Administrators can choose a custom forecast and archive URL, add a write-only encrypted API key, select exact weather coordinates, or disable weather. Disabling weather pauses queued jobs and preserves history.
...
A custom or self-hosted Nominatim base URL can optionally enable debounced autocomplete. Disabling geocoding stops new searches and automatic trip labels; coordinates, saved places, and cached labels remain.
...
Choosing **Disabled** keeps the route on a neutral background. Custom local tile servers are supported. HTTP is accepted only for an explicitly confirmed local/private endpoint.
...
- **Rivian artwork:** new images are mirrored by the server. Disabling it preserves cached artwork and otherwise uses the local placeholder; browsers never fall back to Rivian image hosts.
- **S3-compatible backups:** status is shown here, while endpoint and credential controls remain in **Settings > Backups**.
- **Rivian account:** vehicle connectivity remains managed from **Settings > Vehicles**.
...
Use **Test with synthetic data** before relying on a provider. The test uses a generic location or icon name rather than a real drive.
