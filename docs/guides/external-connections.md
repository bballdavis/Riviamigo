...
Trip geometry remains exact. CARTO tiles are requested through an authenticated Riviamigo proxy, so the tile provider sees the requested map area and the server connection, not each viewer's browser identity. Administrators can add, replace, or explicitly clear a write-only, encrypted **CARTO Basemap key** in the Map basemap connection. Request this product-specific key from [CARTO's Basemap-key page](https://carto.com/basemaps/apikey/); it is different from a CARTO API Access Token. Riviamigo sends the key only from the server to CARTO or a CARTO-hosted custom template, never to an arbitrary custom tile provider or back to a browser. Saving or clearing the Basemap key purges the server cache and gives the browser a fresh first-party tile URL, so reload is not required. Without a key, remote CARTO maps still render with CARTO's watermark and show an informational notice explaining where an administrator can add one. Custom XYZ raster templates require `{z}`, `{x}`, and `{y}`, attribution, and may include an encrypted bearer token. A missing dark template falls back to the light template.
