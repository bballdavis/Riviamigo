...
export function ChartColorField({ value, onChange, label = 'Color', className }: ChartColorFieldProps) {
  ...
  const current = value.mode === 'token' ? getChartColor(value.token) : isDark ? value.dark : value.light;
    ...
    <div className="grid gap-2 rounded-xl border border-border bg-bg-elevated/30 p-3">
      <fieldset>
        ...
        <div className="grid grid-cols-8 gap-1.5" role="radiogroup" aria-label="Theme series colors">
          {CHART_SERIES_TOKENS.map((token, index) => <button key={token} type="button" role="radio" aria-checked={value.mode === 'token' && value.token === token} aria-label={`Series ${index + 1}`} title={`Series ${index + 1}`} onClick={() => onChange({ mode: 'token', token })} className={cn('aspect-square min-h-7 rounded-md border outline-none focus-visible:ring-2 focus-visible:ring-accent', value.mode === 'token' && value.token === token ? 'border-fg ring-2 ring-accent' : 'border-border')} style={{ backgroundColor: getChartColor(token) }} />)}
        </div>
      </fieldset>
      <fieldset>
        <legend className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-fg-tertiary">Semantic and status</legend>
        <div className="flex flex-wrap gap-1.5">
          {SEMANTIC_COLORS.map(({ token, label: itemLabel }) => <button key={token} type="button" onClick={() => onChange({ mode: 'token', token })} className={cn('inline-flex min-h-8 items-center gap-2 rounded-lg border px-2.5 text-xs font-medium outline-none focus-visible:ring-2 focus-visible:ring-accent', value.mode === 'token' && value.token === token ? 'border-accent bg-accent-muted text-fg' : 'border-border bg-bg-surface text-fg-secondary hover:border-border-strong')}><span className="h-3 w-3 rounded-full border border-border" style={{ backgroundColor: getChartColor(token) }} />{itemLabel}</button>)}
        </div>
      </fieldset>
      <button type="button" onClick={() => setPickerOpen(true)} className={cn('flex min-h-10 items-center justify-between gap-3 rounded-lg border px-3 text-left text-sm outline-none focus-visible:ring-2 focus-visible:ring-accent', value.mode === 'custom' ? 'border-accent bg-accent-muted' : 'border-border bg-bg-surface hover:border-border-strong')}>
